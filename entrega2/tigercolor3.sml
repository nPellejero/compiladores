...
let 
	...
	fun funAuxPrev (item_c:tigertemp.temp, assems) =
		let
			...
			val access2 = tigerframe.allocLocal frame true 
...
(*			val _ = print ("Access: "^puntero^"\n")  *)
	...
	fun funAux (item:tigertemp.temp) preAssem (instr::postAssem) =
		...
		(*	val postAssem = List.tl(postAssem) *)
			...
			val preAssem = if (member(miuse,item) orelse member(midef,item)) then preAssem else preAssem @ [instr] 
		(*  val miuse = if equal(intersection(miuse,midef),empty String.compare) then miuse else difference(miuse,midef) esto lo hice pq extrañamente el dst se repite dentro del src en algunos casos... no entiendo pq.*)		
			val (preAssem,instr,fetchTemp) = if member(miuse,item)
							then
								let
								...
								in if member(midef, item) then (preAssem @ fetchIns,newI,miTemp)  else (preAssem @ fetchIns @ [newI],instr,miTemp) end
							else (preAssem,instr,"TERR")   
							...
							then
								let
  								...
  								| (MOVE{assem = a, dst = d, src = s}) => (MOVE{assem = a, dst = miTemp, src = s}) 		
									| _ => instr
									val storeIns = if member(miuse,item) 
																 then (moveIns (tigertree.TEMP miTemp,tigertree.TEMP fetchTemp)) @ [newI] @ storeIns 
																 else [newI] @ storeIns
									val assemsP = List.map (format (fn x=>x)) ([instr] @ storeIns)
									val _ = List.map print assemsP
							 in preAssem @ storeIns end
							else preAssem
